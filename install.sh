...
# param $5 string $TMP_INSTALL_DIR
#
...
InstallSvn() {
	...
	then
		...
		InstallMain $INSTALL_DIR $HOOK_DIR $REPOSITORY $REPOSITORY_DIR $TMP_INSTALL_DIR svn ExampleSvn
	else
		return 1;
...
InstallGit() {
	...
	then
		# Copy and setup all stuff.
		InstallMain $INSTALL_DIR $HOOK_DIR $REPOSITORY $REPOSITORY_DIR $TMP_INSTALL_DIR git ExampleGit
	else
		return 1;
	fi
